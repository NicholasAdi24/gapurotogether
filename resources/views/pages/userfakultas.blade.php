...
<!--    Main Content-->
<!-- ===============================================-->
<h2 class="mb-2 lh-sm">Users Fakultas</h2>
                    ...
                    <div class="card-header p-4 border-bottom border-300 bg-soft">
                        <div class="row g-3 justify-content-between align-items-end">
                            <div class="col-12 col-md">
                                <h4 class="text-900 mb-0" data-anchor="data-anchor">Daftar User Fakultas</h4>
                                <p class="mb-0 mt-2 text-800">Daftar <code>User Fakultas</code> Di Universitas DIponegoro</p>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 text-end">
                                    <button class="btn btn-primary btn-sm px-6 px-sm-8" data-bs-toggle="modal" data-bs-target="#addUser">
                                        <i class="fa fa-plus" aria-hidden="true"></i> Tambah User Fakultas
                                    </button>
                                </div>
                            </div>
                            <br>
                                    ...
                                    <table class="table table-striped table-bordered" id="usersTable">
                                        <thead class="table-light">
                                            <tr>
                                                ...
                                                <th class="sort border-top" data-sort="prodi" width="20%">Fakultas</th>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addUserModalLabel">Add New User Fakultas</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><span class="fas fa-times fs--1"></span></button>
            </div>
        <form action="{{ route('userfakultass.store') }}" method="POST">
            @csrf
        <div class="modal-body">
            ...
            <div class="mb-3">
                ...
                <select class="form-select select2" id="basic-form-role" aria-label="Default select example" name="fakultas">
...
<div class="modal fade" id="editModal" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit User Fakultas</h5>
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    <div class="mb-3">
                        <label>Roles</label>
                        <select id="editFakultas" class="form-select" id="basic-form-role" aria-label="Default select example" name="fakultas">
                            {{-- <option selected="selected">Select Role</option> --}}
                            @foreach ($fakultass as $fakultas)
                                <option value="{{ $fakultas->id }}">{{ $fakultas->nama_fakultas }}</option>
                            @endforeach
...
$(document).ready(function () {
    ...
    $(document).on('click', '.editBtn', function () {
        let id = $(this).data('id');
        $.get("{{ url('userfakultas-show') }}/" + id, function (data) {
            $('#editId').val(data.id);
            $('#editUser').val(data.users_id);
            $('#editFakultas').val(data.fakultas_id);
    ...
    $(document).on('click', '.deleteBtn', function () {
        if (confirm("Delete this user?")) {
            let id = $(this).data('id');
            $.ajax({
                url: "{{ url('userfakultas-destroy') }}/" + id,
                type: "DELETE",
